    ...
    <>
      <PageHeader
        ...
        actions={
          <>
            ...
            {poweredByClaude ? (
              <Badge tone="amber">
                <Icon.spark className="h-3 w-3" /> {t("Powered by Claude Opus 4.8")}
              </Badge>
            ) : (
              <Badge tone="purple">
                <Icon.spark className="h-3 w-3" /> {t("Powered by NEXUS Intelligence")}
              </Badge>
            )}
          </>
        }
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card className="flex h-[560px] overflow-hidden p-0 lg:col-span-2">
          ...
          <div className="mb-3 flex items-center gap-2 border-b pb-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1 text-[11px] text-emerald-500">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> {t("Online · connected to your data")}
              </div>
            </div>
                ...
                <div className={cn("flex max-w-[75%] flex-col gap-1", m.role === "user" && "items-end")}>
                  ...
                  {m.role === "ai" && m.source && (
                    <div className="flex items-center gap-1 px-1 text-[10px] font-medium">
                      ...
                      />
                      <span className={sourceMeta[m.source].className}>{t(sourceMeta[m.source].label)}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            ...
            <input
              ...
              placeholder={busy ? t("NEXUS is thinking…") : t("Ask NEXUS anything…")}
              disabled={busy}
            ...
            {busy && authed ? (
              ...
              >
                <span className="h-2 w-2 rounded-[2px] bg-white" /> {t("Stop")}
              </button>
            ) : (
              <button
                type="submit"
        ...
        <div className="space-y-4">
          <Card>
            <div className="mb-3 text-sm font-semibold">{t("Live Insights")}</div>
            <div className="space-y-3">
              {aiInsights.map((ins) => (
                <div key={ins.id} className="rounded-xl border p-3">
                  <div className="flex items-center justify-between">
                    <Badge tone={typeMeta[ins.type].tone}>{t(typeMeta[ins.type].label)}</Badge>
                    <span className="text-[10px] text-[var(--muted)]">{ins.confidence}% {t("conf.")}</span>
                  </div>
          ...
          <Card>
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
              <Icon.spark className="h-4 w-4 text-gold-400" /> {t("AI Generators")}
            </div>
            <div className="space-y-2">
              {[
                { kind: "kpi" as const, label: t("SMART KPI Set"), icon: Icon.performance, desc: t("Weighted, data-driven KPIs") },
                { kind: "idp" as const, label: t("Individual Development Plan"), icon: Icon.development, desc: t("IDP from competency gaps") },
                { kind: "report" as const, label: t("Executive Report"), icon: Icon.document, desc: t("Narrative status report") },
              ].map((g) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-medium">{t("Generate")} {g.label}</div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      ...
                      <label className="block text-[11px] font-medium text-[var(--muted)]">
                        ...
                        <input
                          ...
                          placeholder={t("e.g. SLA & competency")}
                  ...
                  {gen.kind === "idp" && (
                    <label className="block text-[11px] font-medium text-[var(--muted)]">
                      ...
                      <select
                        ...
                        className="mt-1 w-full rounded-lg border bg-[rgb(var(--surface))] px-2.5 py-1.5 text-[13px] text-[var(--text)] outline-none focus:border-royal-500"
                      >
                        <option value="">{t("Auto — lowest readiness")}</option>
                        {employees.map((e) => (
                          <option key={e.employee} value={e.employee}>
                            {e.employee} · {e.readiness}% {t("ready")}
                          </option>
                        ))}
                      </select>
                    </label>
                  ...
                  {gen.kind === "report" && (
                    <label className="block text-[11px] font-medium text-[var(--muted)]">
                      ...
                      <input
                        ...
                        placeholder={t("e.g. Q3 Board Review")}
                        className="mt-1 w-full rounded-lg border bg-[rgb(var(--surface))] px-2.5 py-1.5 text-[13px] outline-none focus:border-royal-500"
                      />
                    </label>
                  )}
                  <div className="mt-3">
                    {gen.streaming ? (
                      <Btn variant="ghost" onClick={stopGenerator}>
                        <span className="h-2 w-2 rounded-[2px] bg-rose-500" /> {t("Stop")}
                      </Btn>
                    ) : (
                      <Btn variant="primary" onClick={runGenerator}>
                        <Icon.spark className="h-4 w-4" /> {gen.markdown ? t("Regenerate") : t("Generate")}
                      </Btn>
                    )}
                  </div>
                </div>
              ...
              {gen.loading ? (
                <div className="flex items-center gap-1.5 text-sm text-[var(--muted)]">
                  ...
                  <span className="typing-dot h-1.5 w-1.5 rounded-full bg-royal-400" style={{ animationDelay: "150ms" }} />
                  <span className="typing-dot h-1.5 w-1.5 rounded-full bg-royal-400" style={{ animationDelay: "300ms" }} />
                  <span className="ml-1">{t("Generating from live data…")}</span>
                </div>
              ) : gen.markdown ? (
